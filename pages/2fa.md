...
2. Login to the [CCR Identity Management (IDM) portal](https://idm.ccr.buffalo.edu/) and click on
...
3. Click on the "New Token" button and follow the directions to scan the QR
   code using your authenticator application.  Enter the six digit passcode generated by the application into the Identity Management (IDM) portal to confirm you've linked the account correctly.  If so, you'll see the token added and enabled.  If this failed, you'll see the error `Failed to verify token` and you can try again.    
...
If you worked with CCR staff to have the two factor authentication removed from your account, you will need to re-enable it or you won't be able to login to ColdFront or OnDemand or upload new SSH keys.  

1. Login to the [identity management portal](https://idm.ccr.buffalo.edu) and click on the "OTP Tokens" option on the left.  Remove the token(s) you no longer need or the ones associated with the device(s) you no longer have access to.

2. [Follow the instructions](#enabling-two-factor-authentication) above to add a new token by connecting your current device to your account. 

3. Once this is done, click on the "Security" menu on the left and ensure that "Enable two factor authentication" is on.  If it isn't, click the button to enable it.
